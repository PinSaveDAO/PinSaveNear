...
const Post = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      if (posts.includes(item.token_id)) setSaved(true);
      setUserPost(item);
      setIsLoading(false);
    };
    fetchPost();
      ...
      <div className="flex flex-col lg:flex-row mx-auto justify-center mt-24">
        ...
        <div className="mx-4 lg:ml-12 p-4 rounded-md shadow-md lg:w-full flex-col">
          <div>
            ...
            <h3>
              Owned by{" "}
              <span className="font-semibold">{userpost.owner_id}</span>
            </h3>
            <h5
              onClick={() => {
                window.open(
                  `https://explorer.testnet.near.org/accounts/${userpost.owner_id}`
                );
              }}
              className="mb-4 flex text-black/50"
            >
              View on Explorer
            ...
            <p>
              Description:
              <br />
              <span className="italic">{userpost.metadata.description}</span>
            </p>
          </div>
          <button
            onClick={() => savePost()}
            className="flex mt-12 lg:mt-48 font-bold text-lg rounded-lg p-2 transition-all duration-500 bg-gradient-to-l to-blue-200 via-emerald-300 from-blue-500 bg-size-200 bg-pos-0 hover:bg-pos-100"
          >
            <Tray className="my-auto h-6 w-6 mr-2" />{" "}
            {!saved ? "Save for later" : "Unsave post"}
          </button>
        </div>
      </div>
    );
